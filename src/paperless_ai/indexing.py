...
def get_or_create_storage_context(*, rebuild=False):
    ...
    if not AI_LIBS_AVAILABLE:
        logger.warning("AI libraries not available, returning None")
        return None
...
def build_document_node(document: Document):
    """
    Given a Document, returns parsed Nodes ready for indexing.
    """
    if not AI_LIBS_AVAILABLE:
        logger.warning("AI libraries not available, cannot build document node")
        return []
...
def remove_document_docstore_nodes(document: Document, index):
    """
