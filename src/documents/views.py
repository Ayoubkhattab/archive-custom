...
class BulkPermissionMixin:
    ...
    def get_serializer_context(self):
        ...
        permission_name_delete = f"delete_{model_name}"

        user_perms = self._get_object_perms(
            objects=queryset,
            perm_codenames=[permission_name_view, permission_name_change, permission_name_delete],
            actor="users",
        )
        ...
        context["users_delete_perms"] = {
            pk: user_perms[pk][permission_name_delete] for pk in user_perms
        }
        ...
        context["groups_delete_perms"] = {
            pk: group_perms[pk][permission_name_delete] for pk in group_perms
        }
...
class ChatStreamingView(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        min_chars = 200
        if document and len(document_text) < min_chars:
            return HttpResponseBadRequest(
        ...
        if document:
            if len(document_text) <= max_chars:
                ...
                chunk = max_chars // 3
                start = document_text[:chunk]
                mid_start = max(len(document_text) // 2 - chunk // 2, 0)
                middle = document_text[mid_start : mid_start + chunk]
...
class BulkEditView(PassUserMixin):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not user.is_superuser:
            ...
            # For delete: check delete permissions
            # Global delete_document perm (from group) allows deleting any viewable doc
            # Object-level delete_document perm allows deleting that specific doc
            # For other methods: check change permissions (global + object-level)
            if method == bulk_edit.delete:
                has_global_delete = user.has_perm("documents.delete_document")
                if has_global_delete:
                    # Global delete perm: user can delete any doc they can view
                    has_perms = all(
                        has_perms_owner_aware(user, "view_document", doc)
                        for doc in document_objs
                    )
                else:
                    # No global delete: need object-level delete on each doc
                    has_perms = all(
                        has_perms_owner_aware(user, "delete_document", doc)
                        for doc in document_objs
                    )
            else:
            ...
            # check ownership for methods that change the original document
            if (
                (
                    has_perms
                    and method
            ...
            # check global delete permissions for merge/split with delete_originals
            if (
                has_perms
                and (
                    method in [bulk_edit.merge, bulk_edit.split]
