...
class OwnedObjectSerializer(
        ...
        if not self.all_fields:
            try:
                if self.full_perms:
                    ...
                    self.fields.pop("user_can_delete")
    ...
    def get_permissions(self, obj) -> dict:
        return {
            ...
            "delete": {
                "users": self._get_perms(obj, "delete", "users"),
                "groups": self._get_perms(obj, "delete", "groups"),
            },
        }
    ...
    def get_user_can_delete(self, obj) -> bool:
        checker = ObjectPermissionChecker(self.user) if self.user is not None else None
        ...
        return (
            ...
            or (
                self.user is not None
                and (
                    checker.has_perm(f"delete_{model_name}", obj)
                    or self.user.has_perm(f"documents.delete_{model_name}")
                )
            )
        )
    ...
    user_can_delete = SerializerMethodField(read_only=True, required=False)
