...
def set_permissions_for_object(permissions: dict, object, *, merge: bool = False):
    ...
    for action, entry in permissions.items():
        ...
        if "users" in entry:
            ...
            # Collect audit data before applying changes
            if settings.AUDIT_LOG_ENABLED:
                granted = list(users_to_add.values_list("username", flat=True))
                revoked = list(users_to_remove.values_list("username", flat=True)) if users_to_remove.exists() else []
                if granted or revoked:
                    audit_changes[f"object_perm_{action}_users"] = {
                        "type": "permission",
                        "action": action,
                        "granted": granted,
                        "revoked": revoked,
            ...
            if users_to_add.exists():
                for user in users_to_add:
                    ...
                    if action == "delete":
                        # delete gives view and change too
                        assign_perm(
                            f"view_{object.__class__.__name__.lower()}",
                            user,
                            object,
                        ...
                        assign_perm(
                            f"change_{object.__class__.__name__.lower()}",
                            user,
                            object,
                        )
        if "groups" in entry:
            ...
            # Collect audit data before applying changes
            if settings.AUDIT_LOG_ENABLED:
                granted = list(groups_to_add.values_list("name", flat=True))
                revoked = list(groups_to_remove.values_list("name", flat=True)) if groups_to_remove.exists() else []
                if granted or revoked:
                    audit_changes[f"object_perm_{action}_groups"] = {
                        "type": "permission",
                        "action": action,
                        "granted": granted,
                        "revoked": revoked,
    ...
    # Log all permission changes as a single audit entry on the target object
    if settings.AUDIT_LOG_ENABLED and audit_changes:
